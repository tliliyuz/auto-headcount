...
- Web 采集任务复用同表，已实现 `browser_job_batch_discover` 和 `browser_job_collect`；候选人在规划 `browser_candidate_discovery`、`browser_candidate_collect`（范围见 [10-candidate-collection](10-candidate-collection.md)，锚定人才池「互联网技术」分类，先采脱敏画像）、`browser_ingestion_normalize` 和 `browser_candidate_sanitize`。载荷只含来源/外部 ID、用户/设备、契约版本、数字断点和批量上限，不含浏览器 Session、页面正文或联系方式。发现任务按 [`browser-job-batch-discover.task.v2`](contracts/browser-job-batch-discover.task.v2.schema.json) 校验并事务创建幂等详情任务；详情任务按 [`browser-job-collect.task.v1`](contracts/browser-job-collect.task.v1.schema.json) 校验，执行顺序固定为连接预检 → 确定性导航与受限提取 → 白名单回执校验 → 本地沉睡规则 → 加密原始记录与职位幂等 upsert；任一步失败均不得写 `jobs`。
