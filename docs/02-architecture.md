...
- 两条受限契约均通过 `csdn_run_extraction_contract` 执行：`liebide-filtered-job-list-v2` 在当前筛选列表上固定复核“有效推荐数 0、发布时间最近 30 天”，按 `batchSize`/`maxPages` 有界翻页并返回数字断点；`liebide-job-detail-v1` 按发现的外部 ID 确定性导航、等待页面就绪，再返回版本化职位白名单，并独立复核 `active + 发布 7～30 天 + 有效推荐数 0`。因此列表发现的 0～6 天职位只记跳过，不写 `jobs`。auto-headcount 在消费列表断点时做差分发现：读取该来源已入库职位集合，跳过已入库且标题未变的职位（不创建详情任务），按断点续采直到凑满 `batchSize` 个「新增 + 标题变化」职位或列表到底，标题变化由详情事务 upsert 覆盖。交互式验证可显式提交 `browserSessionId`；持久化任务只保存 `userId`、`deviceId`、契约参数和期望职位 ID，由 Relay 在该双重作用域内选择当前活跃页面。两种路径都必须校验 owner 与固定来源，详情路径还必须校验页面职位 ID，不能跨用户、跨设备或猜测其他页面。请求和回执中的未知字段、敏感键、错误域名或 ID 不一致均失败关闭。
...
- auto-headcount 已实现协议校验、Relay 预检/提取客户端、`browser_job_batch_discover`/`browser_job_collect` 调度分发、批次与断点持久化、管理端批量触发、**差分采集**（本批数量=新增+标题变更，已知未变跳过）以及职位事务入库；CSDN-Agent 支持不持久化 session 的设备作用域 active page 路由，并对同一 tab 重复注册的会话按 tabId 去重。Fixture 已验证列表发现、批次拆分、详情复核和幂等行为，既有单职位路径已完成 PostgreSQL 验证；**真实筛选 DOM、整批 PostgreSQL 调度和后台登录态入库已由真实批次 `a6221eb9`（2026-08-14）复验**：差分发现 20 条（`target_reached`）→ 详情突发认领 20/20 入库、0 失败、列表标题与库标题 0 不一致。候选人直传、ingestion ticket 和脱敏流水线仍未实现。
...
- Web 采集任务复用同表，已实现 `browser_job_batch_discover` 和 `browser_job_collect`；候选人在规划 `browser_candidate_discovery`、`browser_candidate_collect`（范围见 [10-candidate-collection](10-candidate-collection.md)，锚定人才池「互联网技术」分类，先采候选人画像——内部存真实姓名、脱敏只针对匹配 LLM 投影）、`browser_ingestion_normalize` 和 `browser_candidate_sanitize`。载荷只含来源/外部 ID、用户/设备、契约版本、数字断点和批量上限，不含浏览器 Session、页面正文或联系方式。发现任务按 [`browser-job-batch-discover.task.v2`](contracts/browser-job-batch-discover.task.v2.schema.json) 校验并事务创建幂等详情任务；详情任务按 [`browser-job-collect.task.v1`](contracts/browser-job-collect.task.v1.schema.json) 校验，执行顺序固定为连接预检 → 确定性导航与受限提取 → 白名单回执校验 → 本地沉睡规则 → 加密原始记录与职位幂等 upsert；任一步失败均不得写 `jobs`。
...
- **阶段一任务 kind（2026-08-14，M3）**：`match_projection_filter` 是阶段一（投影生成 + 硬过滤）的周期自动编排任务，与 `match_pipeline_v2` 同受 `MATCH_AUTOMATION_ENABLED` 门禁。运行前增量选择需（重新）投影的可操作沉睡职位（`selectJobsNeedingProjection`：沉睡口径 + 不存在「generator_version 匹配且 created_at ≥ job.updated_at」的 consumable 投影，内容变化触发重投影），对选定职位生成 `job_match_projections` 并逐 (job, candidate) 落 `match_filter_results`；候选人为 0 或候选人无脱敏详情来源时只产出职位投影（正确空跑），`match_pipeline_v2` 待候选人/脱敏详情就绪后收敛。
...
> **已落地（2026-08-14）**：此前本地集成测试的 `DATABASE_URL` 与 dev docker compose 的 `db` 共享同一 PostgreSQL，真实 scheduler 持续写入 `jobs`/`sync_runs`/`audit_logs` 导致整批集成测试偶发 flaky。现 `scripts/run-integration-tests.mjs` 把 `test:integration` 整体隔离到独立测试库 `auto_headcount_test`：由 `DATABASE_URL` 推导（仅库名替换）、自动建库、清空业务表（保留 `__drizzle_migrations`）、跑迁移，再以测试库 URL 派生 `node --test`；测试文件顺序执行（`--test-concurrency=1`）消除并行写竞争。dev 库 `auto_headcount` 不再被集成测试写入。CHANGELOG 见 `2026-08-14` 条目。
