...
5. 确认插件显示设备在线、浏览器会话存在、当前页面脚本已注册。
...
|---|---:|---:|---|
| Bridge 未启动或不可达 | 有限 | 否 | 启动 Bridge，确认健康后重新检查 |
| 设备离线 | 等待 | 否 | 检查插件和设备绑定，不切换其他账号或设备 |
| 网页未连接/页面脚本未注册 | 否 | 否 | 刷新供应方页面；仍失败则重载插件后再次刷新 |
| 登录失效 | 否 | 否 | 由操作人员人工重新登录 |
| 验证码或异常跳转 | 否 | 否 | 立即停止，不绕过风控 |
| 429 | 否 | 否 | 熔断并等待人工确认预算和恢复时间 |
| 来源域名错误 | 否 | 否 | 导航到授权域名，不放宽白名单 |
| 页面职位 ID 与任务不一致 | 否 | 否 | 检查任务目标和当前页面，不覆盖任务 ID |
