...
| 接口 | 说明 | 请求要点 | 响应要点 |
|---|---|---|---|
| `POST /api/auth/login` | 账号口令登录 | `{ username, password, totpCode? }` | `200 { user, roles }`；失败统一 `401` 文案 |
| `POST /api/auth/logout` | 退出并撤销会话 | 会话 Cookie | `204` |
| `GET /api/auth/me` | 当前主体与角色 | 会话 Cookie | `200 { user, roles }` / `401` |

- 会话标识只存服务端，数据库只保存会话令牌哈希；Cookie 为 `HttpOnly`/`Secure`/`SameSite=Lax`。
- 生产管理员登录与高风险操作须携带并校验 TOTP，未校验时拒绝。
- 登录失败返回统一文案，不区分账号是否存在；连续失败限流锁定。
- 口令校验使用 argon2id/bcrypt，任何接口与日志不得回显口令或口令哈希。
