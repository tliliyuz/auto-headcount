...
- **2026-08-14 真实整批复验完成**：真实猎必得筛选列表 + 登录态 + 差分采集批次 `a6221eb9` 端到端成功——真实筛选 DOM 发现 20 条唯一职位（`stop_reason=target_reached`，跳过已入库未变职位）、整批 PostgreSQL 调度自动完成 20 条详情（突发认领）、后台登录态事务入库 20/20、0 失败、列表标题与库标题 0 不一致；`jobs` 由 22 涨到 40（18 新增 + 2 标题变更更新）。同日排障两项真实批次错误：`BROWSER_SESSION_MISSING`（bridge 会话按 tabId 去重，csdn-agent `BridgeHub.registerSession` 修复）与 `BROWSER_COLLECTION_ARGUMENTS_INVALID`（发现预检缺 `batchSize/maxPages`，Consumer 修复）。至此第 57 行所列"待复验"三项（真实筛选 DOM、整批调度、登录态入库）均已闭合，证据见 [CHANGELOG](../../CHANGELOG.md) 与 [roadmap](../05-roadmap.md)。
