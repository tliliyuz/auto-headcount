...
test("脱敏白名单：只含标题/类别/城市/薪资范围，剔除原始 JD 及一切识别信息", () => {
  ...
  assert.deepEqual(
    ...
    ["category", "city", "salaryRange", "title"],
  );
  ...
  assert.ok(!json.includes("Lovart"), "不包含原始 JD 内嵌公司/品牌名（职责摘要已省略）");
...
test("职责摘要省略：原始 JD 不进入公开 DTO（防止内嵌公司/品牌名泄漏）", () => {
  const view = toMaskedJobView({
    title: "x",
    category: "x",
    city: "x",
  ...
  assert.ok(!("responsibilities" in view), "DTO 不含职责摘要字段");
  assert.ok(!JSON.stringify(view).includes("Lovart"));
});
