...
生产批次固定为 `browser_job_batch_discover → browser_job_collect × N`。运营人工登录并在列表选择“推荐 0 人、发布时间最近 30 天”，然后在管理端选择本批数量。`batchSize` 是差分目标：发现任务读取该来源已入库职位集合，跳过已入库且标题未变的职位（不创建详情任务、不耗详情提取），按合同 `nextPage`/`nextOffset` 数字断点向后翻页直到凑满 `batchSize` 个「新增 + 标题变化」职位或列表到底。详情合同按固定地址自动导航（每次强制跨文档整页加载，避免 SPA 同文档 hash 切换残留上一岗位 DOM）并重新校验职位 ID、标题（与发现阶段列表标题比对，不一致失败关闭）、active、发布 7～30 天和零推荐；标题变化由事务 upsert 覆盖，不产生重复职位。最近 30 天中发布不足 7 天的职位只记跳过，不写 `jobs`。不得要求运营逐个打开详情，也不得用自然语言提示词驱动生产翻页循环。
